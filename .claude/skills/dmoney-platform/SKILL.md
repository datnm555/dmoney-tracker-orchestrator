...
| Gold type (Vàng) DTO + optional `goldTypeId`/`goldQuantity` pair on transaction writes and `goldTypeId`/`goldTypeName`/`goldQuantity` on reads | `src/Application/GoldTypes/Data/GoldTypeResponse.cs`, `src/Application/Gold/Data/GoldSummaryResponse.cs` | `src/api/types.ts` (`GoldTypeResponse`, `GoldSummaryResponse`), `src/api/goldApi.ts`, `src/gold/` |
